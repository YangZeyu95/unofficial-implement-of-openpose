    ...
    def gen_net(self):
        ...
        with tf.variable_scope('train_layers'):
            ...
            with tf.variable_scope('stage1'):
                cpm_net = self.stage_1(inputs=added_layers_out, out_channel_num=self.cpm_channel_num, name='stage1_cpm')
            ...
            with tf.variable_scope('staget'):
                for i in range(self.stage_num - 1):
                    ...
                    cpm_net = self.stage_t(inputs=net, out_channel_num=self.cpm_channel_num, name='stage%d_cpm' % (i + 2))
                    cpm_pre.append(cpm_net)
                    hm_pre.append(hm_net)
                    if i < self.stage_num - 2:
                        net = tf.concat([hm_net, cpm_net, added_layers_out], 3)
        return hm_pre, cpm_pre, added_layers_out
